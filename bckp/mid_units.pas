...
{$IFNDEF DVC}
 ...
 // constructor Create(AOwner: TComponent); override;
  ...
  function  encodeTCdate(tcs: String; var hr,min,sec: Integer): TdateTime;

  function  encodeTCstr(hr,min,sec: Integer): String;
...
function TunitFrm.encodeTCstr(hr, min, sec: Integer): String;
var
 hrs,mins,secs,str: String;
begin
 ...
 function TunitFrm.encodeTCdate(tcs: String; var hr,min,sec: Integer): TdateTime;
 var
 // hr,min,sec: Integer;
  dtm: TdateTime;
 begin
...
begin
 ...
 if trim(filename)=NUL then
  ...
  flname :=copy(extractFileName(filename),1,ps_end);
...

{
  ...
  if showFrames then
   str :=format('%s%s%s%s%s%s%s',[hrs,TC_SEP,mins,TC_SEP,secs,TC_SEP,TC_ZERO])
  else
   str :=format('%s%s%s%s%s',[hrs,TC_SEP,mins,TC_SEP,secs]);
